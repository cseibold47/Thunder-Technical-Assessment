...
#Calculating total shots for each team
GroupedShots %>% group_by(team) %>% summarise(TotalShots = sum(count)) -> TotalShots

#Joining Data and getting shot%
GroupedShots %>% left_join(TotalShots, by = "team") %>% mutate(`Shot%` = count/TotalShots)-> FinalTable
